# v4.8 — SI 与 forward return 的 IC 评估

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 新建 `backtrace/dynamics/dynamics_si_ic.py` — 用滚动 cross-sectional Spearman IC 评估 v4.7 SI 指数 vs 行业 forward 20d/60d 收益的相关性,输出 `si_ic_summary.csv` + `si_ic_timeseries.csv` + 1 HTML + 1 文本汇总。

**Architecture:** 独立 CLI,只读 v4.7 `sector_si.csv` + `kc_estimates.csv` + `data/daily/<code>.csv`,**不动**任何已有文件。复用 `dynamics_state_backtest.py:275-304` 的 Spearman 计算模式,但不引用那个模块(避免循环)。

**Tech Stack:** Python 3.13 / pandas / numpy / scipy / plotly / pytest / tsfresh 全栈环境(`/c/ProgramData/anaconda3/python.exe`)
...
- `dynamics_eigen_analysis.py` / `dynamics_state_backtest.py` 0 行修改(只参考)
- 输出全部 gitignored (`data/dynamics/` + `backtrace/outputs/`)
- `PYTHONIOENCODING=utf-8` 必备;Windows 用 `/c/ProgramData/anaconda3/python.exe`
- 38 tests pass 目标(35 旧 + 3 新)

## 现状(实施前必读)

- `data/dynamics/sector_si.csv` (v4.7 产出) — 128 行业 × 9 列
- `data/projection/kc_estimates.csv` — 全 A 股 ~5000 只,有 `industry_l1` 列(经 `load_industry_lookup` 合并)
- `data/daily/<code>.csv` — 单只票日线,列: `date, open, high, low, close, volume, amount`  (与 `tsfresh_pipeline.load_ohlcva` 一致)
- `data/sw2/members.csv` — 反查 `industry_l1 → sector_name`(沿用 v4.3 `_industry_name_lookup`)
- `dynamics_state_backtest.py:275-304` — 现有 IC 计算模式,本轮**不引用**(避免循环)
- `tests/test_dynamics_eigen.py` 现有 35 测试,新 3 测试追加末尾

---

## Task 1: 3 测试 + 1 新 CLI (`dynamics_si_ic.py`) + README §3.8

**Files:**
- Create: `backtrace/dynamics/dynamics_si_ic.py` (~180 行,6 个函数 + main)
- Modify: `tests/test_dynamics_eigen.py` (追加 3 测试,~50 行)
- Modify: `backtrace/dynamics/README.md` (§3.8 v4.8 节,~30 行)

**Interfaces:**
- `load_sector_si(path: str) -> pd.DataFrame` — 读 sector_si.csv,返回至少含 `industry_l1, SI` 两列
- `load_industry_membership(kc_path: str, sw2_path: str) -> dict[str, list[str]]` — `industry_l1 → [code, ...]`
- `compute_industry_forward_returns(membership: dict, daily_dir: str, dates: pd.DatetimeIndex, horizon: int) -> pd.DataFrame` — 返回 `[date × industry_l1]` 矩阵,值是 forward `horizon` 日收益
- `rolling_cross_sectional_ic(si: dict, fwd_ret: pd.DataFrame, window: int = 60, step: int = 20) -> pd.DataFrame` — 返回 per-window 1 行 (window_end_date, ic, p_value, n_industries)
- `write_si_ic_summary(ts_df: pd.DataFrame, output_path: str) -> None` — 写跨期汇总 2 行(20d / 60d)
- `build_si_ic_html(ts_df: pd.DataFrame, summary_df: pd.DataFrame, si_dict: dict, fwd_ret_60d: pd.DataFrame, output_path: str) -> None` — 1 HTML 3 子图(rolling IC 时序 + 行业 SI vs 累计 forward 60d 散点 + 跨期统计表)
...
PYTHONIOENCODING=utf-8 /c/ProgramData/anaconda3/python.exe -m pytest tests/test_dynamics_eigen.py -k "si_ic" -v
```

**Expected:** 3 个 `ImportError` 或 `pytest.skip`(模块未实现)

### Step 1.3: 创建 `dynamics_si_ic.py` 骨架

创建新文件 `backtrace/dynamics/dynamics_si_ic.py`,基本结构:
...
**Expected:** 38 passed (35 旧 + 3 新)

### Step 1.6: 更新 README §3.8

打开 `backtrace/dynamics/README.md`,在 `### 3.7 v4.7` 节后追加:
...
### Step 1.7: 端到端冒烟(单值 SI 评估)

```bash
cd "C:\Users\yellow\mcp\qtTdx"
PYTHONIOENCODING=utf-8 /c/ProgramData/anaconda3/python.exe backtrace/dynamics/dynamics_si_ic.py --horizons 20
# 期待:exit 0,产出 4 个新文件 (si_ic_summary.csv / si_ic_timeseries.csv / dynsys_si_ic.html / si_ic_summary.txt 路径见 main)
```

**注意**: 端到端跑需要 `data/daily/<code>.csv` 与 v4.7 `sector_si.csv` 都已存在。如果 `sector_si.csv` 不存在,跑 v4.7 先生成。

### Step 1.8: Commit

```bash
cd "C:\Users\yellow\mcp\qtTdx"
git add backtrace/dynamics/dynamics_si_ic.py tests/test_dynamics_eigen.py backtrace/dynamics/README.md
git commit -m "feat(dynamics): v4.8 — SI × forward return 滚动 Spearman IC 评估

- 新 CLI backtrace/dynamics/dynamics_si_ic.py(~150 行)
  - load_sector_si / load_industry_membership / compute_industry_forward_returns
  - rolling_cross_sectional_ic(60 日窗口, 20 日步长)
  - write_si_ic_summary / build_si_ic_html
- 3 新测试 (38/38 pass):
  - test_si_ic_synthetic_perfect (IC ≈ 1)
  - test_si_ic_synthetic_random (IC ≈ 0)
  - test_si_ic_summary_schema (2 horizons × 6 cols)
- 4 新输出文件 (全 gitignored):
  - data/dynamics/si_ic_summary.csv
  - data/dynamics/si_ic_timeseries.csv
  - backtrace/outputs/dynsys_si_ic.html
  - backtrace/outputs/dynsys_si_ic_summary.txt
- README §3.8 新增 v4.8 节
- 数学层 + 3 caller + compute_sector_stability 0 改 (v4.7 闭环验证)"
```
...
- ❌ 修改 3 个现有 caller
- ❌ 修改 `dynamics_eigen_analysis.py` / `dynamics_state_backtest.py` / `_dynamics_core.py`

## 验证清单

- [ ] Step 1.1: 3 测试写入
- [ ] Step 1.2: 3 测试跳过/失败
- [ ] Step 1.3: `dynamics_si_ic.py` 骨架 + 5 函数 + main 写完
- [ ] Step 1.4: 3 测试通过
- [ ] Step 1.5: 38 tests pass
- [ ] Step 1.6: README §3.8 更新
- [ ] Step 1.7: 端到端冒烟(单值 SI 评估)— 4 文件产出
- [ ] Step 1.8: commit
...
| 风险 | 缓解 |
|---|---|
| 行业 member 数小(< 10 只)中位数收益不稳 | 文本汇总列 `n_industries`;每窗口 IC 排除 n_industries < 5 日 |
| `sector_si.csv` 缺失(刚清缓存) | `load_sector_si` 入口 `FileNotFoundError` 提示先跑 v4.7 |
| `data/daily/<code>.csv` 缺失 / 停牌 | `compute_industry_forward_returns` 用 `dropna(how='all')` 跳过空列 |
| IC ≈ 0(类似 state_prop 现象) | 文本汇总明确报告"预测力 ≈ 0",不强行包装 |
| `scipy.stats.spearmanr` 在 2 行业时退化为 ±1 | `n_industries >= 5` 才计算 IC,否则 NaN |
| 滚动窗口 60 日 + 步长 20 日,需要 ≥ 6 个月数据 | README 标注 |

## 与 v4.7 的关系

| 版 | commit | 主题 |
|---|---|---|
| v4.7 | `c63e783` | 行业稳定性指数 SI 单一指标加权 |
| **v4.8** | (本次) | **SI × forward return Rolling IC 评估** |

v4.7 回答"哪些行业最稳",v4.8 回答"稳有没有用" — 业务价值闭环。
