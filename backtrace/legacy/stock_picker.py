...
try:
    ...
    print(f"[OK] 已成功创建自定义板块「{block_name}（{block_code}）」")
except Exception as e:
    # 板块已存在时可能报错，此处捕获异常不中断流程
    print(f"[INFO] 自定义板块创建提示：{e}（若提示已存在，可忽略此信息）")
...
if not target_stocks.empty:
    ...
    try:
        ...
        print(f"\n[OK] 已成功将股票添加至自定义板块「{block_name}（{block_code}）」")
    except Exception as e:
        print(f"\n[FAIL] 添加自定义板块失败：{e}")
    ...
    try:
        ...
        print(f"[FAIL] 消息发送失败：{e}")
else:
    # 无符合条件股票时清空板块
    print("暂无符合条件的股票")
    try:
        tq.send_user_block(block_code=block_code, stocks=[])
        print(f"[OK] 已清空自定义板块「{block_name}（{block_code}）」")
    except Exception as e:
        print(f"[FAIL] 清空自定义板块失败：{e}")
