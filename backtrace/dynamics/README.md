...
#### 6.9.1 `factor_validation.csv` (主表,19 列)
...
| `status` | str | `ok` / `insufficient_data`(`min_n_per_date=10` 默认阈值)|

#### 6.9.3 `factor_validation_by_industry.csv` (8 列,主表按申万二级行业聚合)
...
| `n_obs` | int | 该行业样本数 |
| `n_dates` | int | 该行业交易日数 |
| `ic_mean` / `ic_std` / `status` | float / float / str | 同 `factor_validation_by_year.csv` |

#### 6.9.4 `dynsys_factor_validation_summary.txt` (UTF-8 纯文本)

按 `|ic_mean|` 降序排列输出 top-10 因子 + 显著项 (p<0.05) 全表 + by-year 显著提示 + by-industry top-N + 决策提示(v6 §3.11 决策标准的中文版)。

#### 6.9.5 `dynsys_factor_validation.html` (2×2 plotly)

| (行, 列) | 子图 |
|---|---|
| (1, 1) | Top-10 (factor, horizon) IC mean(bar)|
| (1, 2) | Top-10 Q5-Q1 spread(bar)|
| (2, 1) | top 因子 by-year IC(line plot)|
| (2, 2) | top 因子 by-industry IC(bar)|
...
# 默认:limit=500, horizons=1,5,10,20, 读 4 个 CSV + data/stocks/
...
| 路径 | 内容 |
|---|---|
| `data/dynamics/factor_validation.csv` | main 表:每 (factor, horizon) 一行,列见 §6 |
| `data/dynamics/factor_validation_by_year.csv` | 按年聚合 |
| `data/dynamics/factor_validation_by_industry.csv` | 按申万二级聚合 |
| `data/dynamics/dynsys_factor_validation_summary.txt` | UTF-8 中文汇总(top-10 + by-year + by-industry 显著项 + 决策提示) |
| `backtrace/outputs/dynsys_factor_validation.html` | 2×2 plotly 仪表板:IC / Q5-Q1 / by-year / by-industry |
...
3. **Q1-Q5** = `pd.qcut(factor_t, 5, duplicates='drop')` → 每组 forward return **均值**
4. **Q5 - Q1** = 多空收益差(单调性指标)
...
- 24 个因子并非正交(相关性见 HTML (2,2))— 不要叠加做"alpha 组合",先用单因子评估
